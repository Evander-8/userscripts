...
> **站点已临时下架打赏暴击（幸运奖励）**，这个标签页现在被一层灰色提示盖住，下架的统计暂时看不到。等站点恢复，把脚本里的 `CONFIG.critOnline` 改回 `true` 就全部还原（浮层不再生成，其余逻辑原样可用）。
...
**帖子列表 / 主题页上直接标出来**（与暴击无关，下架期间照常工作）：今天打赏过的作者，名字后面会自动挂一个红底白字的【已打赏】；站点是 ajaxify + 滚动加载，新翻出来的帖子、切页后的列表都会补标（`a[href*="/user/"]` 的文字命中今日打赏名单就标，头像链接不带文字所以不会重复标）
...
- **打赏暴击已下架，面板给出提示**：站点临时下架了打赏暴击（幸运奖励），打赏标签页上盖一层灰色浮层，写明「打赏暴击已下架」。浮层没有关闭入口，点它也不会消失，就是要让人一眼看出这块暂时不能用
- 下架只是显示层的事：中奖率、今日打赏/暴击、最近打赏结果、玩家名单、收支这些照旧计算与渲染，只是被浮层盖住。新增开关 `CONFIG.critOnline`（现为 `false`），站点恢复后改回 `true`，浮层不再生成，其余原样可用
- 帖子列表 / 主题页的【已打赏】标记与暴击无关，下架期间照常工作
